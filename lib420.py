...
class AssetGroup():
    ...
    def __init__(self, *N_assets):
        ...
        self.years = sorted(
            list(set([self.price_hist.index[i].year for i in range(len(self.price_hist))])))
        ...
        self.return_hist_struct = pd.DataFrame([[self.return_hist[asset][self.return_hist.index.year == year].values] for asset in self.assets for year in self.years],
                                               index=pd.MultiIndex.from_tuples(
                                                   [(asset, year) for asset in self.assets for year in self.years]),
    ...
    def yearly_portfolio_struct(self):
        ...
        for year in self.years[1:]:
            ...
            for asset in self.assets:
                portfolio_for_year.append(
                    self.return_hist_struct.loc[asset, year]['Return History'])
        ...
        self.yearly_portfolio = pd.DataFrame(
            port_list, index=self.years[1:], columns=['Portfolio'])
    ...
    def cov_struct(self):
        ...
        for year in self.yearly_portfolio.index:
            cov_dict[year] = pd.DataFrame(
                cov_dict[year], index=self.assets, columns=self.assets)
    ...
    def corr_struct(self):
        ...
        for year in self.yearly_portfolio.index:
            corr_dict[year] = pd.DataFrame(
                corr_dict[year], index=self.assets, columns=self.assets)
    ...
    def min_volatility_alloc_struct(self):  # , cov_struct):
        ...
        for year in self.yearly_portfolio.index:
            ...
            # should be in the order of the assets passed in
            fmv = np.dot((1 / a), y.T)
        ...
        min_volatility_allocations = pd.DataFrame(
            min_vol_alloc_list, index=self.yearly_portfolio.index, columns=self.assets)
    ...
    def portfolio_volatility_struct(self):
        ...
        portfolio_vol_struct = pd.DataFrame(
            vol_list, index=self.yearly_portfolio.index, columns=self.assets)
    ...
    def long_frontier(self, year):
        ...
        for i in range(len(mu)):
            ...
            # [ones(length(v),1).';m.']
            C5 = np.vstack((np.ones((1, len(covar))), [m.T]))
            ...
            sol = cvxopt.solvers.qp(cvxopt.matrix(P.values), cvxopt.matrix(q), cvxopt.matrix(
                G), cvxopt.matrix(h), cvxopt.matrix(A), cvxopt.matrix(b), solver='glpk')
            x = sol['x']
